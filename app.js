...
function feedUrl(kind='all',page=1,year='all'){return route('/feed',new URLSearchParams({kind,page,year}));}
...
function renderDetail(s){
 ...
 app.innerHTML=`<div class="shell detail-layout"><div class="backbar"><a href="${esc(backUrl())}">${icon('back')} 返回动态列表</a><a href="${esc(safeUrl(detail.url))}" target="_blank" rel="noopener noreferrer">原站动态 ${icon('external')}</a></div><div class="workspace">${sidebar('moment')}<section class="feed-column">${card(detail,true)}<section class="comment-section" id="comments"><div class="comment-heading"><h2>评论区<small>原站 ${fmt(detail.counts.commentCount)}</small></h2>${icon('message')}</div><p class="snapshot-note">已保存 ${fmt(detail.archivedComments)} 条评论与回复，含 ${fmt(detail.archivedRoots)} 条主评论。互动量为保存时的记录。</p><nav class="comment-tabs" aria-label="评论排序"><a class="${tab==='all'?'active':''}" href="${esc(changed({tab:'all',cp:1,thread:null,rp:null}))}">全部评论 ${fmt(detail.archivedRoots)}</a><a class="${tab==='hot'?'active':''}" href="${esc(changed({tab:'hot',cp:1,thread:null,rp:null}))}">热门评论 ${fmt(detail.hotCount)}</a></nav>${detail.comments.map(c=>comment(c,detail)).join('')||`<div class="empty">${icon('message')}<p>${tab==='hot'?'尚未保存热门评论。':'尚未保存可展示的评论。'}</p></div>`}${pagination(page,total,'cp')}</section><p class="page-footnote">${date(detail.time)} 发布 · ${date(new Date(data.generatedAt).getTime())} 保存</p></section></div></div>`;
}
...
async function getJSON(path){
 if(!jsonCache.has(path))jsonCache.set(path,fetch('./data/'+path).then(r=>{if(!r.ok)throw new Error('内容暂时无法加载');return r.json();}).catch(e=>{jsonCache.delete(path);throw e;}));
...
async function render(){
 ...
 const onlyThread=sameMoment&&oldState.get('cp')===s.params.get('cp')&&oldState.get('tab')===s.params.get('tab');
 if(lastHash)scrollPositions.set(lastHash,window.scrollY);
 app.setAttribute('aria-busy','true');
 try{
  if(s.id&&data.detailFiles['moment:'+s.id]){
   const base=await getJSON(data.detailFiles['moment:'+s.id]);
   ...
   const comments=pages.length?await getJSON(pages[page-1]):[];
   ...
   if(info){const rp=selectedPage(s.params,'rp',info.pages.length);detail.threadView={info,root:info.root,replies:info.pages.length?await getJSON(info.pages[rp-1]):[],page:rp,total:Math.max(1,info.pages.length)};}
   if(version!==renderVersion)return;
   data.details['moment:'+s.id]=detail;
  }
  if(version!==renderVersion)return;
  if(!onlyThread){if(s.id)renderDetail(s);else renderFeed(s);}
  renderThread(s);
  ...
  lastHash=hash;lastMoment=s.id||'';
 }catch(err){if(version===renderVersion){if(threadDialog.open)threadDialog.close();app.innerHTML='<div class="shell empty">内容暂时无法加载。<button class="text-link" data-action="retry">重新加载</button> · <a href="#/feed">返回列表</a></div>';}}
...
document.addEventListener('change',e=>{if(e.target.id==='year-filter')location.hash=changed({year:e.target.value,page:1});});
...
try{
 const [manifest,feed,media,emotions]=await Promise.all([getJSON('manifest.json'),getJSON('feed.json'),getJSON('media.json'),getJSON('emotions.json')]);
 data={...manifest,feed,media,emotions,details:{...manifest.detailFiles}};
 await render();
}catch(err){app.innerHTML='<div class="shell empty">页面数据暂时无法加载，请刷新重试。</div>';}

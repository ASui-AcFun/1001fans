...
function periodPicker(period){
 const months=[...new Set(data.feed.filter(x=>x.time).map(x=>date(x.time).slice(0,7)))].sort().reverse();
 const years=[...new Set(months.map(x=>x.slice(0,4)))];
 const title=period==='all'?'全部月份':period.length===4?period+' 年':period.slice(0,4)+' 年 '+Number(period.slice(5))+' 月';
 ...
 return `<details class="date-filter"><summary aria-label="筛选年份和月份">${title}${icon('down')}</summary><div class="date-options" role="group" aria-label="选择年份和月份">${option('all','全部月份','date-all')}${years.map(y=>`<section class="date-year"><h3>${option(y,y+' 年 · 全年')}</h3><div class="month-grid">${months.filter(m=>m.startsWith(y)).sort().map(m=>option(m,Number(m.slice(5))+' 月')).join('')}</div></section>`).join('')}</div></details>`;
}
function renderFeed(s){
 ...
 const filtered=data.feed.filter(x=>(kind==='all'||x.kind===kind)&&(period==='all'||date(x.time).startsWith(period))).sort((a,b)=>ascending?a.time-b.time:b.time-a.time);
 ...
 app.innerHTML=`<div class="shell"><section class="hero">${hero?`<img class="hero-art" src="${esc(hero)}" alt="1001Project 绘画作品">`:''}</section><div class="workspace">${sidebar(kind)}<section class="feed-column"><div class="feed-toolbar"><h2 class="feed-heading">${label[kind]}<span>${filtered.length} 条</span></h2><div class="feed-meta"><a class="sort-toggle" href="${esc(changed({order:ascending?'desc':'asc',page:1}))}" title="切换发布时间排序">按发布时间${ascending?'顺序':'倒序'}<span aria-hidden="true">${ascending?'↑':'↓'}</span></a>${periodPicker(period)}</div></div>${pagination(page,total,'page',s.path,'top')}${filtered.slice((page-1)*pageSize,page*pageSize).map(x=>card(x)).join('')||'<div class="card empty">这个筛选下没有内容。</div>'}${pagination(page,total,'page')}<p class="page-footnote">共 ${fmt(data.feed.length)} 条历史记录<br>动态在这里留存，文章与视频回到 AcFun 继续阅读。</p></section></div></div>`;
}
